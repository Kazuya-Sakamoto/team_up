...
// GetAllApplyJobs ...
func GetAllApplyJobs(limit int64, offset int64, userID int64, jobID int64, applyStatusID int64) (ml []*ApplyJob, err error) {
	...
	if applyStatusID != 0 {
		tx = tx.Where("apply_status_id = ?", applyStatusID)
	}
	if userID == 0 && jobID == 0 {
		logs.Error(err)
		tx.Rollback()
